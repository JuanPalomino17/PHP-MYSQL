$(document).ready(function () {
  ...
  $(document).on("click", ".btnEditar", function () {
    ...
    names = fila.find('td:eq(1)').text();
    name_work = fila.find('td:eq(2)').text();
    entry_date = fila.find('td:eq(3)').text();
  ...
  $(document).on("submit", "#formPersonas", function (e) {
    ...
    $.ajax({
      ...
      success: function (data) {
          ...
          if (opcion == 1) {
            tablaPersonas.row.add([id, names, name_work, entry_date]).draw();
            ...
            tablaPersonas.row(fila).data([id, names, name_work, entry_date]).draw();
            $("#modalCRUD").modal("hide");
